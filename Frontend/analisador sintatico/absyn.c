...
A_exp A_CallExpAnonFunc(A_pos pos, A_exp exp, A_expList args)
{
  A_exp p = checked_malloc(sizeof(*p));
  p->kind=A_callExp;
  p->pos=pos;
  p->u.anonfunc.exp=exp;
  p->u.anonfunc.args=args;
  return p;
}

A_exp A_OpExp(A_pos pos, A_oper oper, A_exp left, A_exp right){
  return A_OpExp2(pos, oper, left, right, NULL);
}
...
A_exp A_IfExp(A_pos pos, A_exp test, A_exp then, A_exp elsee){
  return A_IfExp2(pos,test,then,elsee,NULL);  
}
...
A_exp A_CaseExp(A_pos pos, A_exp test, A_exp body, A_exp casee){
  A_exp p = checked_malloc(sizeof(*p));
  p->kind=A_caseExp;
  p->pos=pos;
  p->u.casee.test=test;
  p->u.casee.body=body;
  p->u.casee.casee=casee;
  return p;  
}
...
{
  A_ty p = checked_malloc(sizeof(*p));
  p->kind=A_funcTySymReturn;
  p->parameter=parameter;
  p->returns.sym=returns;
  return p;
}
...
{
  A_ty p = checked_malloc(sizeof(*p));
  p->kind=A_funcTyFuncReturn;
  p->parameter=parameter;
  p->returns.ty=returns;
  return p;
}
