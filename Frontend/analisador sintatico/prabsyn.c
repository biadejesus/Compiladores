...
static void pr_dec(FILE *out, A_dec v, int d) {
  ...
  switch (v->kind) {
    case A_functionDec:
      ...
      fprintf(out, ",\n");
      if (v->u.function.result) {
        indent(out, d+1);
        fprintf(out, "%s,\n", S_name(v->u.function.result));
      }
      pr_exp(out, v->u.function.body, d+1);
      fprintf(out, ")");
      break;
...
static void pr_ty(FILE *out, A_ty v, int d) {
  ...
  switch (v->kind) {
    ...
    case A_funcTyFuncReturn:
      fprintf(out, "funcTyFuncReturn(\n"); 
      pr_paramlist(out, v->parameter, d+1);  
      pr_ty(out, v->returns.ty, d+1);
      fprintf(out, ")");
      break;
    case A_funcTySymReturn:
      fprintf(out, "funcTySymReturn(\n");  
      pr_paramlist(out, v->parameter,d+1);
      fprintf(out, ",\n");
      indent(out, d+1);   
      fprintf(out, "returnTy(%s)", S_name(v->returns.sym->head));
      break;
    default:
      assert(0); 
  } 
...
static void pr_paramlist(FILE *out, A_paramList v, int d) {
  ...
  if (v) {
    ...
    indent(out, d);
    fprintf(out, "%s", S_name(v->head)); fprintf(out, ",\n ");
