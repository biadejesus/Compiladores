    ...
    switch(v->kind) {
        case A_simpleVar: {
            ...
            if(x && x->kind == E_varEntry) {
                ...
                EM_error(v->pos, "simple var expression: variávek não definida %s", S_name(v->u.simple));
        ...
        case A_fieldVar: {
            ...
            if(var.ty->kind != Ty_record) {
                EM_error(v->u.field.var->pos, "field var expression: não é uma variável do tipo de registro");
                return expTy(NULL, Ty_Int());
            } else {
                Ty_fieldList fl;
                ...
                EM_error(v->u.field.var->pos, "field var expression: nenhum campo <%s> no registro", S_name(v->u.field.sym));
                return expTy(NULL, Ty_Int());
            }
        }
        case A_subscriptVar: {
            ...
            if(var.ty->kind != Ty_array) {
                EM_error(v->u.subscript.var->pos, "subscript var expression: não é uma variável do tipo array");
                return expTy(NULL, Ty_Int());
            } else {
                expty index = transExp(venv, tenv, v->u.subscript.exp);
                if(index.ty->kind != Ty_int) {
                    EM_error(v->u.subscript.exp->pos, "subscript var expression: é necessário um inteiro no índice da matriz");
                    return expTy(NULL, Ty_Int());
                }
    ...
    switch(e->kind) {
        ...
        case A_callExp: {
            ...
            if(!fun_entry || (fun_entry->kind != E_funEntry)) {
                EM_error(e->pos, "call expression: tipo não definido %s", S_name(e->u.call.func));
                return expTy(NULL, Ty_Int());
            } else {
                A_expList el;
                Ty_tyList tl;
                for(el = e->u.call.args, tl = fun_entry->u.fun.formals; el && tl; el = el->tail, tl = tl->tail) {
                    ...
                    if(!actual_eq(tl->head, exp.ty)) {
                        EM_error(el->head->pos, "call expression: tipo de argumento não corresponde ao parâmetro");
                        return expTy(NULL, Ty_Int());
                    }
                }
                if(el) {
                    EM_error(el->head->pos, "call expression: muitos argumentos");
                    return expTy(NULL, Ty_Int());
                }
                if(tl) {
                    EM_error(e->pos, "call expression: argumentos insuficientes");
                    return expTy(NULL, Ty_Int());
                }
            ...
            switch(e->u.op.oper) {
                ...
                case A_divideOp: {
                    ...
                    if(left.ty->kind != Ty_int)
                        EM_error(e->u.op.left->pos, "binary operation: um inteiro é requerido");
                    if(right.ty->kind != Ty_int)
                        EM_error(e->u.op.right->pos, "binary operation: um inteiro é requerido");
                    return expTy(NULL, Ty_Int());
                }
                case A_eqOp:
                case A_neqOp: {
                    // pode ser aplicado a inteiros, strings e duas matrizes ou registros
                     // tenha cuidado com os registros, porque nil também é do tipo registro
                    if(!actual_eq(left.ty, right.ty)) {
                        EM_error(e->pos, "operators compare: tipo diferente para comparar");
                    }
                    return expTy(NULL, Ty_Int());
                }
                case A_ltOp:
                ...
                case A_geOp: {
                    if(left.ty->kind != Ty_int || right.ty->kind != Ty_int) {
                        EM_error(e->pos, "binary compare: um inteiro é requerido");
                    }
                    return expTy(NULL, Ty_Int());
                }
            } /* switch */
        ...
        case A_recordExp: {
            ...
            if(!record_typ) {
                EM_error(e->pos, "record expression: tipo não definido");
                return expTy(NULL, Ty_Record(NULL));
            }
            Ty_ty actual = actual_ty(record_typ);
            if(actual->kind != Ty_record) {
                EM_error(e->pos, "record expression: <%s> não é um tipo de registro", S_name(e->u.record.typ));
                return expTy(NULL, Ty_Record(NULL));
            }
            Ty_fieldList ty_fl;
            A_efieldList fl;
            for(fl = e->u.record.fields, ty_fl = actual->u.record; fl && ty_fl; fl = fl->tail, ty_fl = ty_fl->tail) {
                if(fl->head->name != ty_fl->head->name) {
                    EM_error(e->pos, "record expression: <%s> não é um nome de campo válido", S_name(fl->head->name));
                    return expTy(NULL, Ty_Record(NULL));
                }
                expty exp = transExp(venv, tenv, fl->head->exp);
                if(!actual_eq(exp.ty, ty_fl->head->ty)) {
                    EM_error(e->pos, "record expression: ambos os tipos de campo divergem");
                    return expTy(NULL, Ty_Record(NULL));
                }
            }
        ...
        case A_assignExp: {
            ...
            if(!actual_eq(var.ty, exp.ty)) {
                EM_error(e->pos, "assign expression: divergência de tipo entre variável e expressão");
            }
        ...
        case A_ifExp: {
            ...
            if(test.ty->kind != Ty_int) {
                EM_error(e->pos, "condition expression: a seção de teste deve ser inteira");
                return expTy(NULL, Ty_Void());
            }
            ...
            if(e->u.iff.elsee) {
                ...
                if(!actual_eq(then.ty, elsee.ty)) {
                    EM_error(e->pos, "condition expression: a secao then-else deve ser do mesmo tipo");
                    return expTy(NULL, Ty_Void());
                }
                return expTy(NULL, then.ty);
            } else {
                if(then.ty->kind != Ty_void) {
                    EM_error(e->pos, "condition expression: seção then precisa ser void");
                }
                return expTy(NULL, Ty_Void());
            }
        }
        case A_whileExp: {
            ...
            if(test.ty->kind != Ty_int) {
                EM_error(e->u.whilee.test->pos, "while loop: seção test deve produzir um inteiro ");
                return expTy(NULL, Ty_Void());
            }
            inside++; // inside loop
            ...
            if(body.ty->kind != Ty_void) {
                EM_error(e->u.whilee.body->pos, "while loop: a seção do corpo não deve produzir valores");
                return expTy(NULL, Ty_Void());
            }
            return expTy(NULL, Ty_Void());
        }
        case A_forExp: {
            ...
            if(lo.ty->kind != Ty_int) {
                EM_error(e->u.forr.lo->pos, "for loop: parte inferior deve ser inteira");
                return expTy(NULL, Ty_Void());
            }
            if(hi.ty->kind != Ty_int) {
                EM_error(e->u.forr.hi->pos, "for loop: parte superior deve ser inteira");
                return expTy(NULL, Ty_Void());
            }
            S_beginScope(venv);
            ...
            inside--; // outside
            if(body.ty->kind != Ty_void) {
                EM_error(e->u.forr.body->pos, "for loop: o corpo precisa ser void");
                return expTy(NULL, Ty_Void());
            }
            S_endScope(venv);
            return expTy(NULL, Ty_Void());
        ...
        case A_breakExp: {
            if(!inside) {
                EM_error(e->pos, "break expression: expressão break fora do laço");
            }
            return expTy(NULL, Ty_Void());
        }
        case A_letExp: {
        ...
        case A_arrayExp: {
            ...
            if(!array_typ) {
                EM_error(e->pos, "array expression: tipo indefinido %s", S_name(e->u.array.typ));
                return expTy(NULL, Ty_Array(NULL));
            }
            Ty_ty actual = actual_ty(array_typ);
            if(actual->kind != Ty_array) {
                EM_error(e->pos, "array expression: tipo de array necessário, mas foi dado outro %s", S_name(e->u.array.typ));
                return expTy(NULL, Ty_Array(NULL));
            }
            expty size_typ = transExp(venv, tenv, e->u.array.size);
            if(size_typ.ty->kind != Ty_int) {
                EM_error(e->u.array.size->pos, "array expression: inteiro requerido com tamanho de array");
                return expTy(NULL, Ty_Array(NULL));
            }
            expty init_typ = transExp(venv, tenv, e->u.array.init);
            if(!actual_eq(init_typ.ty, actual->u.array)) {
                EM_error(e->u.array.init->pos, "array expression: o tipo de inicialização não corresponde ao tipo fornecido");
                return expTy(NULL, Ty_Array(NULL));
            }
            return expTy(NULL, actual);
        }
...
void  transDec(S_table venv, S_table tenv, A_dec d) {
    switch(d->kind) {
        case A_varDec: {
            ...
            if(dec_ty != NULL) {
                if(!actual_eq(dec_ty, init_exp.ty)) {
                    EM_error(d->pos, "variable declare: tipo de incompatibilidade entre declarar e inicializar");
                    return;
                }
            } else {
                if(init_exp.ty->kind == Ty_nil) {
                    EM_error(d->pos, "variable declare: tipo nil ilegal: nil deve ser atribuído a um tipo de registro explicitamente");
                    return;
                }
            }
        ...
        case A_typeDec: {
            ...
            for(type_list = d->u.type; type_list; type_list = type_list->tail) {
                ...
                for(int i = 0; i < index; i++) {
                    if(typenames[i] == (void *)type_list->head->name) {
                        EM_error(type_list->head->ty->pos, "type declare:tipo de redeclaração <%s>, existem dois tipos com o mesmo nome no mesmo lote (consecutivo) de tipos mutuamente recursivos.", S_name(type_list->head->name));
                    }
                }
            ...
            if(cycle_decl) {
                EM_error(d->pos, "type declare: declaração de tipo de ciclo ilegal: deve conter pelo menos um tipo embutido");
                return;
            }
            return;
        }
        case A_functionDec: {
            ...
            for(fun_list = d->u.function; fun_list; fun_list = fun_list->tail) {
                ...
                if(fun_list->head->result) {
                    ...
                    if(!r) {
                        EM_error(fun_list->head->pos, "function declare: tipo de return indefinido %s", S_name(fun_list->head->result));
                        return;
                    }
                } else {
                    r = Ty_Void();
                ...
                for(fl = fun_list->head->params; fl; fl = fl->tail) {
                    ...
                    if(!ty) {
                        EM_error(fl->head->pos, "function declare: tipo de parametro indefinido %s", S_name(fl->head->typ));
                        return;
                    }
                    if(head) {
                ...
                for(int i = 0; i < index; i++) {
                    if(typenames[i] == (void *)fun_list->head->name) {
                        EM_error(fun_list->head->pos, "type declare: tipo de redeclaração <%s>, existem dois tipos com o mesmo nome no mesmo lote (consecutivo) de tipos mutuamente recursivos.", S_name(fun_list->head->name));
                    }
                }
...
Ty_ty transTy (S_table tenv, A_ty t) {
    switch(t->kind) {
        case A_nameTy: {
            ...
            if(!ty) {
                EM_error(t->pos, "translate name type: tipo indefinido %s", S_name(t->u.name));
                return Ty_Void();
            }
            return ty;
        }
        case A_recordTy: {
            ...
            for(fl = t->u.record; fl; fl = fl->tail) {
                ty = S_look(tenv, fl->head->typ);
                if(!ty) {
                    EM_error(fl->head->pos, "translate record type: tipo indefinido %s", S_name(fl->head->typ));
                    return Ty_Void();
                }
        ...
        case A_arrayTy: {
            ...
            if(!ty) {
                EM_error(t->pos, "translate array type: tipo indefinido %s", S_name(t->u.array));
                return Ty_Void();
            }
            return Ty_Array(ty);
        }
